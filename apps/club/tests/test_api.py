...
def test_followed_catalog_filters_before_pagination_and_search(
    ...
    """Following scope belongs in the database, including counts and search."""
    ...
    objects = (
        ...
        if entity == "club"
            ...
            Team.objects.create(name=f"Team {index:02}", club=club)
            for index, club in enumerate(clubs)
        ]
    )
    getattr(viewer, f"{entity}_follow").add(objects[-1], objects[-2])
    getattr(other, f"{entity}_follow").add(objects[0])
    if authenticated:
        client.force_login(viewer.user)
    response = client.get(
        f"/api/{entity}/{entity}s/",
        {"followed": "true", "search": "29", "page_size": 1},
    )
    assert response.status_code == HTTPStatus.OK
    payload = response.json()
    assert payload["count"] == int(authenticated)
...
def test_club_season_teams_avoid_multiplying_match_joins(
    ...
    """Dense home/away schedules preserve season membership without a fanout join."""
    graph = _make_club_graph(club_name="Dense club")
    roster = Team.objects.create(name="Roster", club=graph.club)
    home = Team.objects.create(name="Home", club=graph.club)
    away = Team.objects.create(name="Away", club=graph.club)
    Team.objects.create(name="Absent", club=graph.club)
    TeamData.objects.create(team=roster, season=graph.season)
    for _ in range(4):
        for host, visitor in (
            (home, graph.opponent_team),
            (graph.opponent_team, away),
            (roster, home),
            (away, roster),
        ):
            Match.objects.create(
    ...
    queryset = club_teams(graph.club, graph.season)
    with django_assert_num_queries(1):
        assert [team.name for team in queryset] == ["Away", "Home", "Roster"]
    # Query counts alone miss the quadratic rows generated by reverse joins.
    assert "LEFT OUTER JOIN" not in str(queryset.query)
    assert "SELECT DISTINCT" not in str(queryset.query)


def test_club_match_day_is_public_complete_and_scoped(client: Client) -> None:
    """Include both venues and all statuses without overview's ten-row limit."""
    graph = _make_club_graph(club_name="Match Day Club")
    team = Team.objects.create(name="Home", club=graph.club)
    day = timezone.localdate()
    midnight = timezone.make_aware(datetime.combine(day, time.min))
    fixtures = []
    for index in range(12):
        match = Match.objects.create(
            home_team=team if index % 2 == 0 else graph.opponent_team,
            away_team=graph.opponent_team if index % 2 == 0 else team,
            season=graph.season,
            start_time=midnight + timedelta(minutes=index),
        )
        MatchData.objects.filter(match_link=match).update(
            status=("finished", "active", "upcoming")[index % 3]
        )
        fixtures.append(str(match.id_uuid))
    for instant in (midnight - timedelta(seconds=1), midnight + timedelta(days=1)):
        Match.objects.create(
            home_team=team,
            away_team=graph.opponent_team,
            season=graph.season,
    ...
    response = client.get(
        f"/api/club/clubs/{graph.club.id_uuid}/match-day/", {"date": day.isoformat()}
    )
    assert response.status_code == HTTPStatus.OK
    payload = response.json()
    assert payload["date"] == day.isoformat()
    assert payload["club"]["id_uuid"] == str(graph.club.id_uuid)
    assert [match["id_uuid"] for match in payload["matches"]] == fixtures
    assert {match["status"] for match in payload["matches"]} == {
        "active",
        "upcoming",
        "finished",
    }
...
@pytest.mark.parametrize("value", ["not-a-date", "2026-02-30", "9999-12-31"])
def test_club_match_day_rejects_invalid_dates(client: Client, value: str) -> None:
    """Invalid days return an API error instead of a server error."""
    club = Club.objects.create(name="Date validation")
    response = client.get(f"/api/club/clubs/{club.id_uuid}/match-day/", {"date": value})
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_club_match_day_defaults_to_today_and_can_be_empty(client: Client) -> None:
    """An unattended screen can follow today without choosing a season."""
    club = Club.objects.create(name="Empty club")
    response = client.get(f"/api/club/clubs/{club.id_uuid}/match-day/")
    assert response.status_code == HTTPStatus.OK
    assert response.json()["date"] == timezone.localdate().isoformat()
