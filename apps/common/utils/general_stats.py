...
async def general_stats(match_datas):
    ...
    team_goal_stats = {}
    for goal_type in goal_types:
        goals_for = await Shot.objects.filter(match_data__in=match_datas, shot_type=goal_type, for_team=True, scored=True).acount()
        goals_against = await Shot.objects.filter(match_data__in=match_datas, shot_type=goal_type, for_team=False, scored=True).acount()
