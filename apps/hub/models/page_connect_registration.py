...
class PageConnectRegistration(models.Model):
    ...
    id_uuid = models.UUIDField(  # type: ignore[type-arg]
        primary_key=True,
        default=uuidv7,
        editable=False,
    )
    player = models.ForeignKey(  # type: ignore[type-arg]
        player_model_string,
        on_delete=models.CASCADE,
        related_name="page_connect_registrations",
    )
    page = models.CharField(max_length=255)  # type: ignore[type-arg]
    registration_date = models.DateTimeField(auto_now_add=True)  # type: ignore[type-arg]
