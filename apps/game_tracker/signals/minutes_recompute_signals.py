"""Signals that schedule minutes-played recomputation when timeline data changes."""
...
from apps.game_tracker.services.match_minutes_recompute import (
    schedule_match_minutes_recompute,
)


FINISHED_MATCH_MINUTES_RECOMPUTE_DELAY_SECONDS = 30
...
def _match_data_pre_save(
    ...
    if not instance.pk:
        instance._previous_status_for_minutes_recompute = None  # type: ignore[attr-defined]
        return
    ...
    instance._previous_status_for_minutes_recompute = previous_status  # type: ignore[attr-defined]
...
def _match_data_post_save(
    ...
    **kwargs: object,
    ...
    """Ensure we recompute minutes shortly after a match finishes."""
    if instance.status != "finished":
        return

    previous_status = getattr(instance, "_previous_status_for_minutes_recompute", None)
    if created or previous_status != "finished":
        schedule_match_minutes_recompute(
            match_data_id=str(instance.id_uuid),
            countdown_seconds=FINISHED_MATCH_MINUTES_RECOMPUTE_DELAY_SECONDS,
        )
...
def _shot_changed(sender: type[Shot], instance: Shot, **kwargs: object) -> None:
    ...
    if match_data_id:
        schedule_match_minutes_recompute(match_data_id=match_data_id)
