...
async def get_time(match_data, current_part):
    ...
    if part:
        ...
        try:
            active_pause = await Pause.objects.aget(match_data=match_data, active=True, match_part=current_part)
        except Pause.DoesNotExist:
            active_pause = False
        
        # calculate all the time in pauses that are not active anymore
        pauses = await sync_to_async(list)(Pause.objects.filter(match_data=match_data, active=False, match_part=current_part))
        pause_time = 0
        for pause in pauses:
